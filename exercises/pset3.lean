...
--   | cons _ p ih => simp [toList, length, ih]
...
theorem nodup_length_le_card {l : List V} (hl : l.Nodup) :
  ...
  calc
    l.length = l.toFinset.card := by rw [List.toFinset_card_of_nodup hl]
    _        ≤ Fintype.card V := by simp [Finset.card_le_univ l.toFinset]
...
theorem length_dropTo_le {u v : V} (p : Walk (G := G) u v) (x : V)
  ...
  | cons h_adj p_rest ih =>
    simp [dropTo, length]
    split
    case cons.isTrue h_eq =>
      cases h_eq
      simp [length]
    case cons.isFalse h_neq =>
      simp [toList] at hx
      cases hx with
      | inl h_is_u =>
        simp [h_is_u] at h_neq
      | inr h_in_rest =>
        apply le_trans (ih h_in_rest)
        simp
...
theorem reachable_symm {a b : V} :
  ...
  | nil => exact ⟨Walk.nil⟩
  | cons h_adj p_rest ih =>
    let h_adj' := G.symm h_adj
    obtain ⟨p_rest'⟩ := ih
    exact ⟨Walk.append p_rest' (Walk.cons h_adj' Walk.nil)⟩
